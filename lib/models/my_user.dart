class MyUser {
  MyUser({
    required this.image,
    required this.address,
    required this.name,
    required this.about,
    required this.createdAt,
    required this.lastActive,
    required this.id,
    required this.isOnline,
    required this.email,
    required this.pushToken,
    required this.phone,
    required this.is_student,
    required this.lessons_num,
    required this.student_id,
    required this.level,
    required this.male,
    required this.jan_1,
    required this.jan_2,
    required this.jan_3,
    required this.jan_4,
    required this.jan_5,
    required this.jan_6,
    required this.jan_7,
    required this.jan_8,
    required this.jan_9,
    required this.jan_10,
    required this.jan_11,
    required this.jan_12,
    required this.feb_1,
    required this.feb_2,
    required this.feb_3,
    required this.feb_4,
    required this.feb_5,
    required this.feb_6,
    required this.feb_7,
    required this.feb_8,
    required this.feb_9,
    required this.feb_10,
    required this.feb_11,
    required this.feb_12,
    required this.mar_1,
    required this.mar_2,
    required this.mar_3,
    required this.mar_4,
    required this.mar_5,
    required this.mar_6,
    required this.mar_7,
    required this.mar_8,
    required this.mar_9,
    required this.mar_10,
    required this.mar_11,
    required this.mar_12,
    required this.apr_1,
    required this.apr_2,
    required this.apr_3,
    required this.apr_4,
    required this.apr_5,
    required this.apr_6,
    required this.apr_7,
    required this.apr_8,
    required this.apr_9,
    required this.apr_10,
    required this.apr_11,
    required this.apr_12,
    required this.may_1,
    required this.may_2,
    required this.may_3,
    required this.may_4,
    required this.may_5,
    required this.may_6,
    required this.may_7,
    required this.may_8,
    required this.may_9,
    required this.may_10,
    required this.may_11,
    required this.may_12,
    required this.jun_1,
    required this.jun_2,
    required this.jun_3,
    required this.jun_4,
    required this.jun_5,
    required this.jun_6,
    required this.jun_7,
    required this.jun_8,
    required this.jun_9,
    required this.jun_10,
    required this.jun_11,
    required this.jun_12,
    required this.jul_1,
    required this.jul_2,
    required this.jul_3,
    required this.jul_4,
    required this.jul_5,
    required this.jul_6,
    required this.jul_7,
    required this.jul_8,
    required this.jul_9,
    required this.jul_10,
    required this.jul_11,
    required this.jul_12,
    required this.aug_1,
    required this.aug_2,
    required this.aug_3,
    required this.aug_4,
    required this.aug_5,
    required this.aug_6,
    required this.aug_7,
    required this.aug_8,
    required this.aug_9,
    required this.aug_10,
    required this.aug_11,
    required this.aug_12,
    required this.sep_1,
    required this.sep_2,
    required this.sep_3,
    required this.sep_4,
    required this.sep_5,
    required this.sep_6,
    required this.sep_7,
    required this.sep_8,
    required this.sep_9,
    required this.sep_10,
    required this.sep_11,
    required this.sep_12,
    required this.oct_1,
    required this.oct_2,
    required this.oct_3,
    required this.oct_4,
    required this.oct_5,
    required this.oct_6,
    required this.oct_7,
    required this.oct_8,
    required this.oct_9,
    required this.oct_10,
    required this.oct_11,
    required this.oct_12,
    required this.nov_1,
    required this.nov_2,
    required this.nov_3,
    required this.nov_4,
    required this.nov_5,
    required this.nov_6,
    required this.nov_7,
    required this.nov_8,
    required this.nov_9,
    required this.nov_10,
    required this.nov_11,
    required this.nov_12,
    required this.dec_1,
    required this.dec_2,
    required this.dec_3,
    required this.dec_4,
    required this.dec_5,
    required this.dec_6,
    required this.dec_7,
    required this.dec_8,
    required this.dec_9,
    required this.dec_10,
    required this.dec_11,
    required this.dec_12,
    required this.likes,
    required this.paid,
  });

  late final String image;
  late final String address;
  late final String name;
  late final String about;
  late final String createdAt;
  late final String lastActive;
  late final String id;
  late final bool isOnline;
  late final String email;
  late final String pushToken;
  late final String phone;
  late final bool is_student;
  late final int lessons_num;
  late final String student_id;
  late final int level;
  late final bool male;
  late final jan_1;
  late final jan_2;
  late final jan_3;
  late final jan_4;
  late final jan_5;
  late final jan_6;
  late final jan_7;
  late final jan_8;
  late final jan_9;
  late final jan_10;
  late final jan_11;
  late final jan_12;
  late final feb_1;
  late final feb_2;
  late final feb_3;
  late final feb_4;
  late final feb_5;
  late final feb_6;
  late final feb_7;
  late final feb_8;
  late final feb_9;
  late final feb_10;
  late final feb_11;
  late final feb_12;
  late final mar_1;
  late final mar_2;
  late final mar_3;
  late final mar_4;
  late final mar_5;
  late final mar_6;
  late final mar_7;
  late final mar_8;
  late final mar_9;
  late final mar_10;
  late final mar_11;
  late final mar_12;
  late final apr_1;
  late final apr_2;
  late final apr_3;
  late final apr_4;
  late final apr_5;
  late final apr_6;
  late final apr_7;
  late final apr_8;
  late final apr_9;
  late final apr_10;
  late final apr_11;
  late final apr_12;
  late final may_1;
  late final may_2;
  late final may_3;
  late final may_4;
  late final may_5;
  late final may_6;
  late final may_7;
  late final may_8;
  late final may_9;
  late final may_10;
  late final may_11;
  late final may_12;
  late final jun_1;
  late final jun_2;
  late final jun_3;
  late final jun_4;
  late final jun_5;
  late final jun_6;
  late final jun_7;
  late final jun_8;
  late final jun_9;
  late final jun_10;
  late final jun_11;
  late final jun_12;
  late final jul_1;
  late final jul_2;
  late final jul_3;
  late final jul_4;
  late final jul_5;
  late final jul_6;
  late final jul_7;
  late final jul_8;
  late final jul_9;
  late final jul_10;
  late final jul_11;
  late final jul_12;
  late final aug_1;
  late final aug_2;
  late final aug_3;
  late final aug_4;
  late final aug_5;
  late final aug_6;
  late final aug_7;
  late final aug_8;
  late final aug_9;
  late final aug_10;
  late final aug_11;
  late final aug_12;
  late final sep_1;
  late final sep_2;
  late final sep_3;
  late final sep_4;
  late final sep_5;
  late final sep_6;
  late final sep_7;
  late final sep_8;
  late final sep_9;
  late final sep_10;
  late final sep_11;
  late final sep_12;
  late final oct_1;
  late final oct_2;
  late final oct_3;
  late final oct_4;
  late final oct_5;
  late final oct_6;
  late final oct_7;
  late final oct_8;
  late final oct_9;
  late final oct_10;
  late final oct_11;
  late final oct_12;
  late final nov_1;
  late final nov_2;
  late final nov_3;
  late final nov_4;
  late final nov_5;
  late final nov_6;
  late final nov_7;
  late final nov_8;
  late final nov_9;
  late final nov_10;
  late final nov_11;
  late final nov_12;
  late final dec_1;
  late final dec_2;
  late final dec_3;
  late final dec_4;
  late final dec_5;
  late final dec_6;
  late final dec_7;
  late final dec_8;
  late final dec_9;
  late final dec_10;
  late final dec_11;
  late final dec_12;
  late final likes;
  late final paid;

  MyUser.fromJson(Map<String, dynamic> json) {
    image = json['image'] ?? '';
    address = json['address'] ?? '';
    name = json['name'] ?? '';
    about = json['about'] ?? '';
    createdAt = json['created_at'] ?? '';
    lastActive = json['last_active'] ?? '';
    id = json['id'] ?? '';
    isOnline = json['is_online'] ?? '';
    email = json['email'] ?? '';
    pushToken = json['push_token'] ?? '';
    phone = json['phone'] ?? '';
    is_student = json['is_student'] ?? '';
    lessons_num = json['lessons_num'] ?? '';
    student_id = json['student_id'] ?? '';
    level = json['level'] ?? '';
    male = json['male'] ?? '';
    jan_1 = json['jan_1'];
    jan_2 = json['jan_2'];
    jan_3 = json['jan_3'];
    jan_4 = json['jan_4'];
    jan_5 = json['jan_5'];
    jan_6 = json['jan_6'];
    jan_7 = json['jan_7'];
    jan_8 = json['jan_8'];
    jan_9 = json['jan_9'];
    jan_10 = json['jan_10'];
    jan_11 = json['jan_11'];
    jan_12 = json['jan_12'];
    feb_1 = json['feb_1'];
    feb_2 = json['feb_2'];
    feb_3 = json['feb_3'];
    feb_4 = json['feb_4'];
    feb_5 = json['feb_5'];
    feb_6 = json['feb_6'];
    feb_7 = json['feb_7'];
    feb_8 = json['feb_8'];
    feb_9 = json['feb_9'];
    feb_10 = json['feb_10'];
    feb_11 = json['feb_11'];
    feb_12 = json['feb_12'];
    mar_1 = json['mar_1'];
    mar_2 = json['mar_2'];
    mar_3 = json['mar_3'];
    mar_4 = json['mar_4'];
    mar_5 = json['mar_5'];
    mar_6 = json['mar_6'];
    mar_7 = json['mar_7'];
    mar_8 = json['mar_8'];
    mar_9 = json['mar_9'];
    mar_10 = json['mar_10'];
    mar_11 = json['mar_11'];
    mar_12 = json['mar_12'];
    apr_1 = json['apr_1'];
    apr_2 = json['apr_2'];
    apr_3 = json['apr_3'];
    apr_4 = json['apr_4'];
    apr_5 = json['apr_5'];
    apr_6 = json['apr_6'];
    apr_7 = json['apr_7'];
    apr_8 = json['apr_8'];
    apr_9 = json['apr_9'];
    apr_10 = json['apr_10'];
    apr_11 = json['apr_11'];
    apr_12 = json['apr_12'];
    may_1 = json['may_1'];
    may_2 = json['may_2'];
    may_3 = json['may_3'];
    may_4 = json['may_4'];
    may_5 = json['may_5'];
    may_6 = json['may_6'];
    may_7 = json['may_7'];
    may_8 = json['may_8'];
    may_9 = json['may_9'];
    may_10 = json['may_10'];
    may_11 = json['may_11'];
    may_12 = json['may_12'];
    jun_1 = json['jun_1'];
    jun_2 = json['jun_2'];
    jun_3 = json['jun_3'];
    jun_4 = json['jun_4'];
    jun_5 = json['jun_5'];
    jun_6 = json['jun_6'];
    jun_7 = json['jun_7'];
    jun_8 = json['jun_8'];
    jun_9 = json['jun_9'];
    jun_10 = json['jun_10'];
    jun_11 = json['jun_11'];
    jun_12 = json['jun_12'];
    jul_1 = json['jul_1'];
    jul_2 = json['jul_2'];
    jul_3 = json['jul_3'];
    jul_4 = json['jul_4'];
    jul_5 = json['jul_5'];
    jul_6 = json['jul_6'];
    jul_7 = json['jul_7'];
    jul_8 = json['jul_8'];
    jul_9 = json['jul_9'];
    jul_10 = json['jul_10'];
    jul_11 = json['jul_11'];
    jul_12 = json['jul_12'];
    aug_1 = json['aug_1'];
    aug_2 = json['aug_2'];
    aug_3 = json['aug_3'];
    aug_4 = json['aug_4'];
    aug_5 = json['aug_5'];
    aug_6 = json['aug_6'];
    aug_7 = json['aug_7'];
    aug_8 = json['aug_8'];
    aug_9 = json['aug_9'];
    aug_10 = json['aug_10'];
    aug_11 = json['aug_11'];
    aug_12 = json['aug_12'];
    sep_1 = json['sep_1'];
    sep_2 = json['sep_2'];
    sep_3 = json['sep_3'];
    sep_4 = json['sep_4'];
    sep_5 = json['sep_5'];
    sep_6 = json['sep_6'];
    sep_7 = json['sep_7'];
    sep_8 = json['sep_8'];
    sep_9 = json['sep_9'];
    sep_10 = json['sep_10'];
    sep_11 = json['sep_11'];
    sep_12 = json['sep_12'];
    oct_1 = json['oct_1'];
    oct_2 = json['oct_2'];
    oct_3 = json['oct_3'];
    oct_4 = json['oct_4'];
    oct_5 = json['oct_5'];
    oct_6 = json['oct_6'];
    oct_7 = json['oct_7'];
    oct_8 = json['oct_8'];
    oct_9 = json['oct_9'];
    oct_10 = json['oct_10'];
    oct_11 = json['oct_11'];
    oct_12 = json['oct_12'];
    nov_1 = json['nov_1'];
    nov_2 = json['nov_2'];
    nov_3 = json['nov_3'];
    nov_4 = json['nov_4'];
    nov_5 = json['nov_5'];
    nov_6 = json['nov_6'];
    nov_7 = json['nov_7'];
    nov_8 = json['nov_8'];
    nov_9 = json['nov_9'];
    nov_10 = json['nov_10'];
    nov_11 = json['nov_11'];
    nov_12 = json['nov_12'];
    dec_1 = json['dec_1'];
    dec_2 = json['dec_2'];
    dec_3 = json['dec_3'];
    dec_4 = json['dec_4'];
    dec_5 = json['dec_5'];
    dec_6 = json['dec_6'];
    dec_7 = json['dec_7'];
    dec_8 = json['dec_8'];
    dec_9 = json['dec_9'];
    dec_10 = json['dec_10'];
    dec_11 = json['dec_11'];
    dec_12 = json['dec_12'];
    likes = json['likes'];
    paid = json['paid'] ?? '';
  }

  Map<String, dynamic> toJson() {
    final data = <String, dynamic>{};
    data['image'] = image;
    data['address'] = address;
    data['name'] = name;
    data['about'] = about;
    data['created_at'] = createdAt;
    data['last_active'] = lastActive;
    data['id'] = id;
    data['is_online'] = isOnline;
    data['email'] = email;
    data['push_token'] = pushToken;
    data['phone'] = phone;
    data['is_student'] = is_student;
    data['lessons_num'] = lessons_num;
    data['student_id'] = student_id;
    data['level'] = level;
    data['male'] = male;
    data['jan_1'] = jan_1;
    data['jan_2'] = jan_2;
    data['jan_3'] = jan_3;
    data['jan_4'] = jan_4;
    data['jan_5'] = jan_5;
    data['jan_6'] = jan_6;
    data['jan_7'] = jan_7;
    data['jan_8'] = jan_8;
    data['jan_9'] = jan_9;
    data['jan_10'] = jan_10;
    data['jan_11'] = jan_11;
    data['jan_12'] = jan_12;
    data['feb_1'] = feb_1;
    data['feb_2'] = feb_2;
    data['feb_3'] = feb_3;
    data['feb_4'] = feb_4;
    data['feb_5'] = feb_5;
    data['feb_6'] = feb_6;
    data['feb_7'] = feb_7;
    data['feb_8'] = feb_8;
    data['feb_9'] = feb_9;
    data['feb_10'] = feb_10;
    data['feb_11'] = feb_11;
    data['feb_12'] = feb_12;
    data['mar_1'] = mar_1;
    data['mar_2'] = mar_2;
    data['mar_3'] = mar_3;
    data['mar_4'] = mar_4;
    data['mar_5'] = mar_5;
    data['mar_6'] = mar_6;
    data['mar_7'] = mar_7;
    data['mar_8'] = mar_8;
    data['mar_9'] = mar_9;
    data['mar_10'] = mar_10;
    data['mar_11'] = mar_11;
    data['mar_12'] = mar_12;
    data['apr_1'] = apr_1;
    data['apr_2'] = apr_2;
    data['apr_3'] = apr_3;
    data['apr_4'] = apr_4;
    data['apr_5'] = apr_5;
    data['apr_6'] = apr_6;
    data['apr_7'] = apr_7;
    data['apr_8'] = apr_8;
    data['apr_9'] = apr_9;
    data['apr_10'] = apr_10;
    data['apr_11'] = apr_11;
    data['apr_12'] = apr_12;
    data['may_1'] = may_1;
    data['may_2'] = may_2;
    data['may_3'] = may_3;
    data['may_4'] = may_4;
    data['may_5'] = may_5;
    data['may_6'] = may_6;
    data['may_7'] = may_7;
    data['may_8'] = may_8;
    data['may_9'] = may_9;
    data['may_10'] = may_10;
    data['may_11'] = may_11;
    data['may_12'] = may_12;
    data['jun_1'] = jun_1;
    data['jun_2'] = jun_2;
    data['jun_3'] = jun_3;
    data['jun_4'] = jun_4;
    data['jun_5'] = jun_5;
    data['jun_6'] = jun_6;
    data['jun_7'] = jun_7;
    data['jun_8'] = jun_8;
    data['jun_9'] = jun_9;
    data['jun_10'] = jun_10;
    data['jun_11'] = jun_11;
    data['jun_12'] = jun_12;
    data['jul_1'] = jul_1;
    data['jul_2'] = jul_2;
    data['jul_3'] = jul_3;
    data['jul_4'] = jul_4;
    data['jul_5'] = jul_5;
    data['jul_6'] = jul_6;
    data['jul_7'] = jul_7;
    data['jul_8'] = jul_8;
    data['jul_9'] = jul_9;
    data['jul_10'] = jul_10;
    data['jul_11'] = jul_11;
    data['jul_12'] = jul_12;
    data['aug_1'] = aug_1;
    data['aug_2'] = aug_2;
    data['aug_3'] = aug_3;
    data['aug_4'] = aug_4;
    data['aug_5'] = aug_5;
    data['aug_6'] = aug_6;
    data['aug_7'] = aug_7;
    data['aug_8'] = aug_8;
    data['aug_9'] = aug_9;
    data['aug_10'] = aug_10;
    data['aug_11'] = aug_11;
    data['aug_12'] = aug_12;
    data['sep_1'] = sep_1;
    data['sep_2'] = sep_2;
    data['sep_3'] = sep_3;
    data['sep_4'] = sep_4;
    data['sep_5'] = sep_5;
    data['sep_6'] = sep_6;
    data['sep_7'] = sep_7;
    data['sep_8'] = sep_8;
    data['sep_9'] = sep_9;
    data['sep_10'] = sep_10;
    data['sep_11'] = sep_11;
    data['sep_12'] = sep_12;
    data['oct_1'] = oct_1;
    data['oct_2'] = oct_2;
    data['oct_3'] = oct_3;
    data['oct_4'] = oct_4;
    data['oct_5'] = oct_5;
    data['oct_6'] = oct_6;
    data['oct_7'] = oct_7;
    data['oct_8'] = oct_8;
    data['oct_9'] = oct_9;
    data['oct_10'] = oct_10;
    data['oct_11'] = oct_11;
    data['oct_12'] = oct_12;
    data['nov_1'] = nov_1;
    data['nov_2'] = nov_2;
    data['nov_3'] = nov_3;
    data['nov_4'] = nov_4;
    data['nov_5'] = nov_5;
    data['nov_6'] = nov_6;
    data['nov_7'] = nov_7;
    data['nov_8'] = nov_8;
    data['nov_9'] = nov_9;
    data['nov_10'] = nov_10;
    data['nov_11'] = nov_11;
    data['nov_12'] = nov_12;
    data['dec_1'] = dec_1;
    data['dec_2'] = dec_2;
    data['dec_3'] = dec_3;
    data['dec_4'] = dec_4;
    data['dec_5'] = dec_5;
    data['dec_6'] = dec_6;
    data['dec_7'] = dec_7;
    data['dec_8'] = dec_8;
    data['dec_9'] = dec_9;
    data['dec_10'] = dec_10;
    data['dec_11'] = dec_11;
    data['dec_12'] = dec_12;
    data['likes'] = likes;
    data['paid'] = paid;
    return data;
  }
}
